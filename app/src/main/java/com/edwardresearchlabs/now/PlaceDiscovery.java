...
public final class PlaceDiscovery {
    ...
    public static void discover(double lat, double lon, Callback callback) {
        EXECUTOR.execute(() -> {
            try {
                callback.onSuccess(discoverBlocking(lat, lon));
            } catch (Exception e) {
    ...
    public static List<Candidate> discoverBlocking(double lat, double lon) throws Exception {
        ...
        conn.setRequestProperty("User-Agent", "NOW-EdwardResearchLabs/0.2.1 personal-pilot");
        ...
        for (Candidate c : out) {
            ...
            for (Candidate u : unique) {
                if (u.name.equalsIgnoreCase(c.name)) {
                    duplicate = true;
                    break;
                }
            }
